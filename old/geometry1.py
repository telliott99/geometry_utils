...
def make_circle(ax,Q,r,fc='none',ec='k',ls='-'):
...
def scatter_points(ax,pL,c='k'):
    X,Y = getXY(pL)
    ax.scatter(X,Y,s=POINT_SZ,c=c)
...
Let a be side opposite A and so on
Let P be the point on a opposite A and so on

We find the correct point R on side c as follows
AR = y
RB = x
b/y = a/x

y is the distance on the same side as b, i.e. |AR|
...
def get_incenter_and_bisectors(pL):
    ...
    # point where bisector hits opposite side
    # bisector of C hits c at point R
    R = get_point_by_fractional_length([A,B],b/(a+b))
    
    # similarly we can do:
    P = get_point_by_fractional_length([B,C],c/(b+c))
...
# perhaps modify this to indicate which is vertex (A)
def get_angle(pL):
    A,B,C = pL
    a = get_length([B,C])
    b = get_length([A,C])
...
def draw_broken_chord_figure(ax):
    ...
    draw_line_segment(ax,[A,B])
    draw_line_segment(ax,[B,G])
    draw_line_segment(ax,[M,D])
    return fD
